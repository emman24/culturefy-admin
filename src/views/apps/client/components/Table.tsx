...
const renderClient = (row: IClient) => {
    if (row.client_image) {
        return (
            <AvatarWithImageLink href={`/project/?clientId=${row.id}`}>
                <CustomAvatar src={row.client_image} sx={{ mr: 3, width: 34, height: 34 }} />
            </AvatarWithImageLink>
        )
    } else {
        return (
            <AvatarWithoutImageLink href={`/project/?clientId=${row.id}`}>
                <CustomAvatar
                    skin='light'
                    color={row.avatarColor || 'primary'}
                    sx={{ mr: 3, width: 34, height: 34, fontSize: '1rem' }}
                >
                    {getInitials(row.client_name ? row.client_name : 'client')}
                </CustomAvatar>
